...
import { getState, onStateChange } from '../services/player';
import { getSettings, saveSettings, getFavorites, addFavorite, removeFavorite } from '../services/db';
import type { Station } from '../types/station';
import type { AppSettings } from '../types/storage';

const PLAY_ICON_SVG = '<svg viewBox="0 0 24 24" class="menu-icon" aria-hidden="true"><polygon points="5 3 19 12 5 21 5 3" fill="currentColor"/></svg>';
const PAUSE_ICON_SVG = '<svg viewBox="0 0 24 24" class="menu-icon" aria-hidden="true"><rect x="6" y="4" width="4" height="16" fill="currentColor"/><rect x="14" y="4" width="4" height="16" fill="currentColor"/></svg>';
const STAR_FILL = '\u2605';
const STAR_EMPTY = '\u2606';
...
function setupListeners(): void {
  ...
  if (retryBtn) {
    retryBtn.addEventListener('click', () => { void loadStations(currentLocale); });
  }

  document.querySelectorAll('.station-tab').forEach((tab) => {
    tab.addEventListener('click', () => {
      const locale = tab.getAttribute('data-locale') || 'all';
  ...
  document.addEventListener('click', (e) => {
    ...
    if (target.closest('.station-row__play')) {
      if (id === currentPlayId && getState() === 'playing') {
...
const dashboardDiag = { add: (_msg: string): void => { /* noop */ } };

(function setupDashboardDiagnostics(): void {
  if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
    dashboardDiag.add = (msg: string) => { console.log('🎨', msg); };
  }
})();
...
function renderStationRow(st: Station): string {
  ...
  const isPlaying = isSelected && getState() === 'playing';
  ...
  return `<div class="station-row${isSelected ? ' is-active' : ''}" data-station-id="${st.id}">
    <div class="station-row__info">
      <button class="station-row__play${isPlaying ? ' is-active' : ''}" type="button" aria-label="${isPlaying ? 'Pause' : 'Play'}">
        ${isPlaying ? PAUSE_ICON_SVG : PLAY_ICON_SVG}
      </button>
