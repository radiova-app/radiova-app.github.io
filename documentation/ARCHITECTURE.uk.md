...
| Шлях | Призначення |
|---|---|
| `docs/` | Опублікований вихід GitHub Pages (відстежується). Автоматично генерується виробничою збіркою. Не редагуйте вручну. |
| `dist/` | Локальний вихід збірки. Не відстежується — створюється `npm run build`. |
| `test-results/` | Артефакти верифікації браузера (скріншоти). Створюються `scripts/verify-*.mjs`. Не відстежуються. |
| `src/__fixtures__/` | Тимчасові фікстури лінту для `tests/lint-docs.test.ts`. Створюються та видаляються під час виконання тесту. Не відстежуються. Виключені з лінту та збірки. |
| `PROMPT-*.local.md` | Локальні підказки Codex/редактора. Не відстежуються. |

Повний набір правил ігнорування дивіться у `.gitignore`.
